...
torch::Tensor mbwq_linear_exl2_forward_cuda(
	...
	if (use_cublas || size_m > MAX_Q_GEMM_ROWS){
        // Reconstruct FP16 matrix and using cuBLAS for gemm
        auto fp_w = mbwq_linear_exl2fp_weight_cuda(
                       qweight,
		               scales,
		               zeros,
		               qperm,
...
torch::Tensor gba_dequantize_weight(
    ...
    if (!use_mbw) {
        ...
        return mbwq_linear_exl2fp_weight_cuda(qweight, qscales, qzeros, q_perm, q_group_map, rows);
    }
}
...
torch::Tensor make_group_map(torch::Tensor q_groups, int64_t num_qrows_64) {
    ...
    auto cpu_q_groups = q_groups.cpu();
    auto q_groups_ptr = cpu_q_groups.data_ptr<int16_t>();

    for (int i = 0; i < num_groups; i++) {
        int bits = q_groups_ptr[i * 2];
        int qrows;
        if (i < num_groups - 1) {
            qrows = q_groups_ptr[i * 2 + 3] - q_groups_ptr[i * 2 + 1];
        } else {
            qrows = num_qrows - q_groups_ptr[i * 2 + 1];
        }
    ...
    return torch::tensor(group_map, torch::TensorOptions()
                        .dtype(torch::kInt16)
                        .device(q_groups.device()));
}